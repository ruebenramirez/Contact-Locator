...
if (isset($_POST['submitted']) && isset($_POST['confirm']) && $_POST['confirm'] == 1) {
	...
	$sqlDeleteTerritoryRegionAssoc = "DELETE FROM territory_regions WHERE region_id = '".$_REQUEST['id']."'";
	mysql_query($sqlDeleteTerritoryRegionAssoc) or die('died removing territory_region associations: '.mysql_error());
	
	//delete this region
	$sqlDeleteRegion = "DELETE FROM regions WHERE id='".$_REQUEST['id']."'";
	mysql_query($sqlDeleteRegion) or die('died deleting region: '.mysql_error());
	
	//redirect to index.php
	header('location: index.php');
}
if(isset($_POST['submitted']) && !isset($_POST['confirm'])) {
	$submittedError = '<div class="validation_error">Please check the "confirm delete" checkbox in order to delete this region.</div>';
}
...
if (isset($_REQUEST['id'])) {
	...
	while($region = mysql_fetch_object($resultRegionInfo)) {
		$id = $region->id;
		$name = $region->name;
	}
}
...
Are you sure you want to delete region '<?php echo $name;?>'?
