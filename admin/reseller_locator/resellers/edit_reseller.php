...
function validID($id=null) {
	$sql = "SELECT id FROM resellers WHERE id='".$id."';";
	$results = mysql_query($sql) or die('died looking up id: '.mysql_error());
...
<script type="text/javascript">
	...
	function formLocationUpdate() {
		...
		if (reseller_location == 'int') {
			...
			$('#latlonInputField').load('assists/form_latlon.php', {us: 0});
			...
			$('#zipInputField').load('assists/form_zip.php', {us: 1 <?php $myReseller->printjQueryZip(); ?>}, function() {
...
//				alert('just tried to trim the zip field.  Zip code text field contents: "'+ $("#zip").val()) + '"';
			...
			$('#latlonInputField').load('assists/form_latlon.php', {us: 1 <?php $myReseller->printjQueryLatLon(); ?> }, function(event) {
				geocodeHandler();
			});
...
<form method="POST" action="">
	...
	<input name="rb_location" type="radio" value="us"
	<?php
		if (!empty($_POST)) {
			// values have been posted so use that location..
			if (isset($_POST['rb_location']) && $_POST['rb_location'] == "us") {
				echo 'checked="checked"';
			}
		} else {
			// no values submitted, so use the db values to determine reseller location
			if(isUS($myReseller->country)) {
				echo 'checked="checked"';
			}
		}
	?>
	...
	<input name="rb_location" type="radio" value="int"
	<?php
		if (!empty($_POST)) {
			// values have been posted so use that location..
			if(isset($_POST['rb_location']) && $_POST['rb_location'] != 'us'){
