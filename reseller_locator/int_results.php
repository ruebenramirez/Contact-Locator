...
<a href="int_resellers.php?regionid=<?php echo $_GET['region']?>">< BACK</a>
...
<div class="hr"></div>
	...
	if($product != 'all') {
		$sqlGetResellers = "SELECT `resellers`.id, `resellers`.company, `resellers`.website, `resellers`.email, `resellers`.contact, `resellers`.address, `resellers`.city, `resellers`.state, `resellers`.zip, `resellers`.phone, `resellers`.fax,`resellers`.specialistDealer "
    ...
    <div class="reseller_box">
	  	<div class="reseller">
			...
			?>
	  		...
	  		<?php if(!empty($reseller['website'])) { 
	  			?>
				<div class="subtitle_bold">
					<a href="<?php echo $reseller['website'];?>"><?php echo $reseller['company'];?></a>
				</div><br>
	  			<?php
			} else {
				?>
				<div class="subtitle_bold">
					<?php echo $reseller['company'];?>
				</div><br>
				<?php 
			}
	...
	<div align="right" style="margin-bottom:-15px;margin-top:10px;">
		<a href="#top">
			<img src="../images/top.jpg" alt="top" width="54" height="25" border="0" />
		</a>
	</div>
    <div class="hr"></div>
	<?php
